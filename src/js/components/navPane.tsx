...
export const NavPane: React.FC<INavPaneProps> = (props) => {
  ...
  return (
    <Stack horizontal horizontalAlign="space-between" styles={navHeaderStyle} tokens={{ childrenGap: "10vw" }}>
      <Link styles={nameStyles} href="https://github.com/07kshitij/07kshitij.github.io">
        @07kshitij
      </Link>

      <Stack horizontal tokens={{ childrenGap: "1vw" }}>
        <DefaultButton onClick={aboutScroll} styles={buttonStyles}>
          About
        </DefaultButton>
        <DefaultButton onClick={experienceScroll} styles={buttonStyles}>
          Experience
        </DefaultButton>
        <DefaultButton styles={buttonStyles} href="/blog">
          Blog
        </DefaultButton>
        <DefaultButton onClick={contactScroll} styles={buttonStyles}>
          Contact
        </DefaultButton>
        <DefaultButton
          styles={buttonStyles}
          target="_blank"
          href="https://drive.google.com/file/d/1RL6-qTxNXJYvgBFHcEZPf3xgd9_IPzSt/view?usp=sharing"
        >
          Resume
        </DefaultButton>
      </Stack>
